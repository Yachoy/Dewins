...
from Dewins.ui.NodeGraph.errors import NodePropertyError
...
class NodeModel(object):
    ...
    def to_dict(self):
        ...
        """
        icon: pathlib.PosixPath = self.icon

        node_dict = self.__dict__.copy()

        node_dict["icon"] = icon.__str__() #TODO WHY ARE YOU SAVE ICON AS A PosixPath?!?!??!?
                                           # Okey, may be self.__dict__ must to call to the attrs as a __str__, or str type, but it isn't is
...
class NodeGraphModel(object):
    ...
    def add_port_accept_connection_type(
        ...
        for key in keys:
            if not isinstance(connection_data, set): #TODO its could be mistake, but its also problem when this condition is absent
                if key not in list(connection_data.keys()):
                    connection_data[key] = {}
                    connection_data = connection_data[key]
